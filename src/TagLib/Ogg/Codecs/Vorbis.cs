	...
	public class Vorbis : Codec, IAudioCodec
	{
		...
		public override TimeSpan GetDuration(long firstGranularPosition, long lastGranularPosition)
		{
			return header.sample_rate == 0
				? TimeSpan.Zero
				: TimeSpan.FromSeconds((double) (lastGranularPosition - firstGranularPosition)/(double) header.sample_rate);
		}
		...
		{
			get { return (int) ((float) header.bitrate_nominal/ 1000f + 0.5); }
		}
		...
		{
			get { return string.Format("Vorbis Version {0} Audio", header.vorbis_version); }
		}
