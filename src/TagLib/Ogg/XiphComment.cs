	...
	public class XiphComment : TagLib.Tag, IEnumerable<string>
	{
		...
		public override uint Year
		{
			get
			{
				...
				uint value;
				return (text != null && uint.TryParse(text.Length > 4 ? text.Substring(0, 4) : text, out value))
					? value
					: 0;
			}
		...
		public override uint TrackCount
		{
			get
			{
				...
				if ((text = GetFirstField("TRACKNUMBER")) != null &&
				    (values = text.Split('/')).Length > 1 && uint.TryParse(values[1], out value))
					return value;
		...
		public override uint Disc
		{
			get
			{
				...
				if (text != null && (values = text.Split('/')).Length > 0 &&
				    uint.TryParse(values[0], out value))
					return value;
		...
		public override uint DiscCount
		{
			get
			{
				...
				if ((text = GetFirstField("DISCNUMBER")) != null &&
				    (values = text.Split('/')).Length > 1 && uint.TryParse(values[1], out value))
					return value;
		...
		public override uint BeatsPerMinute
		{
			get
			{
				...
				return (text != null && double.TryParse(text, out value) && value > 0)
